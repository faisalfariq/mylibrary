$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, product_tags) {
                product_tags_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_tags.product_tag_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-product-tags/'+product_tags.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-product-tags/'+product_tags.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+product_tags.id+'" class="text-danger btn-delete-product-tag"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_tags.product_tag_slug + '</td>' +
                                            '<td>' + moment(product_tags.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#product-tags-list-field').html(product_tags_list)
        ...
        $('.btn-delete-product-tag').on('click', function(e){
            e.preventDefault()
            let product_tag_name = $(this).closest('td').contents().filter(function() {
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this product tag ? \n Product tag name : "+ product_tag_name,
                icon: "warning",
                buttons: "Ok",
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-product-tags/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-product-tags';
                                }, 1000); //
                            }else{
                                iziToast.error({
