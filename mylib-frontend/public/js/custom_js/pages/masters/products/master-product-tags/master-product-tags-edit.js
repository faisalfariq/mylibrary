$(document).ready(function(){
    ...
    function validateForm(validationErrors) {
        ...
        Object.keys(validationErrors).forEach(function (fieldName) {
            ...
            var errorMessage = '';
            if(fieldName == 'product_tag_name'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
            if(fieldName == 'product_tag_slug'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
    ...
    $('#form-edit-master-product-tags').on('submit', function(e){
        e.preventDefault();

        var product_tag_id = $("#product_tag_id").val();

        var form_data = new FormData(this);

        $.ajax({
            url : '/master-product-tags/'+ product_tag_id,
            type : 'post',
            data : form_data,
            dataType: 'json',
            cache: false,
            ...
            success: function(response) {
                if(response.status == true){
                    ...
                    setTimeout(function() {
                        window.location.href = '/master-product-tags/' + product_tag_id;
                    }, 1000); //
                }else{
                    iziToast.error({
