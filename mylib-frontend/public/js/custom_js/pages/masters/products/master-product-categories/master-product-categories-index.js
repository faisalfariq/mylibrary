$(document).ready(function () {
    ...
    function displayResults(response) {
        var product_categories_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, product_categories) {
                product_categories_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_categories.product_category_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-product-categories/'+product_categories.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-product-categories/'+product_categories.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+product_categories.id+'" class="text-danger btn-delete-product-category"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_categories.product_category_slug + '</td>' +
                                            '<td>' + moment(product_categories.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#product-categories-list-field').html(product_categories_list)
        ...
        $('.btn-delete-product-category').on('click', function(e){
            e.preventDefault()
            let product_category_name = $(this).closest('td').contents().filter(function() {
            ...
            swal({
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-product-categories/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-product-categories';
                                }, 1000); //
                            }else{
                                iziToast.error({
