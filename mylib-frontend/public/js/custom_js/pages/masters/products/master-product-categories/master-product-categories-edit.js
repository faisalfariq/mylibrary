$(document).ready(function(){
    ...
    function validateForm(validationErrors) {
        ...
        Object.keys(validationErrors).forEach(function (fieldName) {
            ...
            var errorMessage = '';
            if(fieldName == 'product_category_name'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
            if(fieldName == 'product_category_slug'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
    ...
    $('#form-edit-master-product-categories').on('submit', function(e){
        e.preventDefault();
        ...
        $.ajax({
            url : '/master-product-categories/'+ product_category_id,
            type : 'post',
            data : form_data,
            dataType: 'json',
            cache: false,
            ...
            success: function(response) {
                if(response.status == true){
                    ...
                    setTimeout(function() {
                        window.location.href = '/master-product-categories/' + product_category_id;
                    }, 1000); //
                }else{
                    iziToast.error({
