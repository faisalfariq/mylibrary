$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, products) {
                products_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + products.product_sku +
                                                '<div class="table-links">' +
                                                    '<a href="master-products/'+products.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-products/'+products.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+products.id+'" class="text-danger btn-delete-product-tag"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + products.product_name + '</td>' +
                                            '<td>' + products.product_categories.product_category_name + '</td>' +
                                            '<td>' + products.product_brands.product_brand_name + '</td>' +
                                            '<td>' + Number(products.product_price).toFixed(0) + '</td>' +
                                            '<td>' + moment(products.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#products-list-field').html(products_list)
        ...
        $('.btn-delete-product-tag').on('click', function(e){
            e.preventDefault()
            let product_sku = $(this).closest('td').contents().filter((_, node) => node.nodeType === 3).text().trim();
