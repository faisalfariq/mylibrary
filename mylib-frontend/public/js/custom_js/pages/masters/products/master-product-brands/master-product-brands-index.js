$(document).ready(function () {
    ...
    function displayResults(response) {
        var product_brands_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, product_brands) {
                product_brands_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_brands.product_brand_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-product-brands/'+product_brands.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-product-brands/'+product_brands.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+product_brands.id+'" class="text-danger btn-delete-product-brand"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + product_brands.product_brand_slug + '</td>' +
                                            '<td>' + moment(product_brands.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#product-brands-list-field').html(product_brands_list)
        ...
        $('.btn-delete-product-brand').on('click', function(e){
            e.preventDefault()
            let product_brand_name = $(this).closest('td').contents().filter(function() {
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this product brand ? \n Product brand name : "+ product_brand_name,
                icon: "warning",
                buttons: "Ok",
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-product-brands/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-product-brands';
                                }, 1000); //
                            }else{
                                iziToast.error({
