$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, apps) {
                apps_list +=     '<tr>' +
                                            '<td>' +
                                            ...
                                            '<td>' + apps.app_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-apps/'+apps.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-apps/'+apps.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+apps.id+'" class="text-danger btn-delete-app"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + moment(apps.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#apps-list-field').html(apps_list)
        ...
        $('.btn-delete-app').on('click', function(e){
            e.preventDefault()
            let app_name = $(this).closest('td').contents().filter(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-apps';
                                }, 1000); //
                            }else{
                                iziToast.error({
