$(document).ready(function(){
    ...
    function validateForm(validationErrors) {
        ...
        Object.keys(validationErrors).forEach(function (fieldName) {
            ...
            var errorMessage = '';
            if(fieldName == 'channel_name'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
            if(fieldName == 'channel_id_jubelio'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
    ...
    $('#form-edit-master-channels').on('submit', function(e){
        e.preventDefault();
        ...
        $.ajax({
            url : '/master-channels/'+ channel_id,
            type : 'post',
            data : form_data,
            dataType: 'json',
            cache: false,
            ...
            success: function(response) {
                if(response.status == true){
                    ...
                    setTimeout(function() {
                        window.location.href = '/master-channels/' + channel_id;
                    }, 1000); //
                }else{
                    iziToast.error({
