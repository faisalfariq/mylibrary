$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, channels) {
                channels_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + channels.channel_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-channels/'+channels.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-channels/'+channels.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+channels.id+'" class="text-danger btn-delete-channel"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (channels.channel_id_jubelio ?? '-') + '</td>' +
                                            '<td>' + moment(channels.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#channels-list-field').html(channels_list)
        ...
        $('.btn-delete-channel').on('click', function(e){
            e.preventDefault()
            let channel_name = $(this).closest('td').contents().filter(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-channels';
                                }, 1000); //
                            }else{
                                iziToast.error({
