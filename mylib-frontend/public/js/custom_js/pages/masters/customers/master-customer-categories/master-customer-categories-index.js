$(document).ready(function () {
    ...
    function loadResults(page) {
        ...
        $.ajax({
            url: "/master-customer-categories",
            type: "GET",
            data: {
                'page' : page,
                'keyword': keyword
    ...
    function displayResults(response) {
        var customer_categories_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, customer_categories) {
                customer_categories_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + customer_categories.customer_category_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-customer-categories/'+customer_categories.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-customer-categories/'+customer_categories.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+customer_categories.id+'" class="text-danger btn-delete-customer-category"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + moment(customer_categories.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#customer-categories-list-field').html(customer_categories_list)
        ...
        $('.btn-delete-customer-category').on('click', function(e){
            e.preventDefault()
            let customer_category_name = $(this).closest('td').contents().filter(function() {
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this customer category ? \n Customer category name : "+ customer_category_name,
                icon: "warning",
                buttons: "Ok",
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-customer-categories/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-customer-categories';
                                }, 1000); //
                            }else{
                                iziToast.error({
