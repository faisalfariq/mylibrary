$(document).ready(function () {
    ...
    function loadResults(page) {
        ...
        $.ajax({
            url: "/master-customer-recipients",
            type: "GET",
            data: {
                'page' : page,
                'keyword': keyword
    ...
    function displayResults(response) {
        var customer_recipients_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, customer_recipients) {
                customer_recipients_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + customer_recipients.shipping_full_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-customer-recipients/'+customer_recipients.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-customer-recipients/'+customer_recipients.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+customer_recipients.id+'" class="text-danger btn-delete-customer-recipients"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (customer_recipients.shipping_phone ?? '-') + '</td>' +
                                            '<td>' + (customer_recipients.shipping_sub_district ?? '-') + '</td>' +
                                            '<td>' + moment(customer_recipients.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#customer-recipients-list-field').html(customer_recipients_list)
        ...
        $('.btn-delete-customer-recipients').on('click', function(e){
            e.preventDefault()
            let customer_recipient_name = $(this).closest('td').contents().filter(function() { 
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this customer recipient ? \n Customer recipient name : "+ customer_recipient_name,
                icon: "warning",
                buttons: "Ok",
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-customer-recipients/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-customer-recipients';
                                }, 1000); //
                            }else{
                                iziToast.error({
