$(document).ready(function(){
    ...
    function validateForm(validationErrors) {
        ...
        Object.keys(validationErrors).forEach(function (fieldName) {
            ...
            var errorMessage = '';
            if(fieldName == 'cust_acc_name'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
            if(fieldName == 'cust_acc_phone'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
            if(fieldName == 'cust_acc_email'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += message;
                });
            }
    ...
    $('#form-edit-master-customer-accounts').on('submit', function(e){
        e.preventDefault();
        ...
        $.ajax({
            url : '/master-customer-accounts/'+ cust_acc_id,
            type : 'post',
            data : form_data,
            dataType: 'json',
            cache: false,
            ...
            success: function(response) {
                if(response.status == true){
                    ...
                    setTimeout(function() {
                        window.location.href = '/master-customer-accounts/' + cust_acc_id;
                    }, 1000); //
                }else{
                    iziToast.error({
