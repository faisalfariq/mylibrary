$(document).ready(function () {
    ...
    function displayResults(response) {
        var customer_accounts_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, customer_accounts) {
                customer_accounts_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + customer_accounts.cust_acc_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-customer-accounts/'+customer_accounts.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-customer-accounts/'+customer_accounts.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+customer_accounts.id+'" class="text-danger btn-delete-customer-accounts"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (customer_accounts.cust_acc_email ?? '-') + '</td>' +
                                            '<td>' + (customer_accounts.cust_acc_phone ?? '-') + '</td>' +
                                            '<td>' + moment(customer_accounts.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#customer-accounts-list-field').html(customer_accounts_list)
        ...
        $('.btn-delete-customer-accounts').on('click', function(e){
            e.preventDefault()
            let customer_account_name = $(this).closest('td').contents().filter(function() { 
            ...
            swal({
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-customer-accounts/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-customer-accounts';
                                }, 1000); //
                            }else{
                                iziToast.error({
