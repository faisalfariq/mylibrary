$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, customers) {
                customers_list +=     '<tr>' +
                                            '<td>' +
                                            ...
                                            '<td>' + customers.customer_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-customers/'+customers.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-customers/'+customers.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+customers.id+'" class="text-danger btn-delete-customers"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (customers.customer_phone ?? '-') + '</td>' +
                                            '<td>' + moment(customers.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#customers-list-field').html(customers_list)
        ...
        $('.btn-delete-customers').on('click', function(e){
            e.preventDefault()
            let customer_name = $(this).closest('td').contents().filter(function() { 
