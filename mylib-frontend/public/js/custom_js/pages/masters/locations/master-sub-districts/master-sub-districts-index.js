$(document).ready(function () {
    ...
    function displayResults(response) {
        var sub_districts_list = '';

        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, sub_districts) {
                sub_districts_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + sub_districts.sub_district_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-sub-districts/'+sub_districts.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-sub-districts/'+sub_districts.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+sub_districts.id+'" class="text-danger btn-delete-sub-districts"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (sub_districts.cities.city_name ?? '-') + '</td>' +
                                            '<td>' + (sub_districts.post_code ?? '-') + '</td>' +
                                            '<td>' + moment(sub_districts.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#sub-districts-list-field').html(sub_districts_list)
        ...
        $('.btn-delete-sub-districts').on('click', function(e){
            e.preventDefault()
            let sub_district_name = $(this).closest('td').contents().filter(function() { 
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this sub-districts ? \n Sub-districts name : "+ sub_district_name,
                icon: "warning",
                buttons: "Ok",
                ...
                if (result) {
                    $.ajax({
                        type : "delete",
                        url : "/master-sub-districts/"+ this.id,
                        headers: {
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-sub-districts';
                                }, 1000); //
                            }else{
                                iziToast.error({
