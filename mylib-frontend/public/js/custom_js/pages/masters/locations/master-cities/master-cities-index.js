$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, cities) {
                cities_list +=     '<tr>' +
                                            '<td>' +
                                            ...
                                            '<td>' + cities.city_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-cities/'+cities.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-cities/'+cities.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+cities.id+'" class="text-danger btn-delete-city"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (cities.provinces.province_name ?? '-') + '</td>' +
                                            '<td>' + moment(cities.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#cities-list-field').html(cities_list)
        ...
        $('.btn-delete-city').on('click', function(e){
            e.preventDefault()
            let city_name = $(this).closest('td').contents().filter(function() {
