$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, provinces) {
                provinces_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + provinces.province_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-provinces/'+provinces.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-provinces/'+provinces.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+provinces.id+'" class="text-danger btn-delete-province"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + moment(provinces.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#provinces-list-field').html(provinces_list)
        ...
        $('.btn-delete-province').on('click', function(e){
            e.preventDefault()
            let province_name = $(this).closest('td').contents().filter(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-provinces';
                                }, 1000); //
                            }else{
                                iziToast.error({
