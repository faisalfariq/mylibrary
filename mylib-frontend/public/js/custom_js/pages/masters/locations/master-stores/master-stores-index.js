$(document).ready(function () {
    ...
    function displayResults(response) {
        ...
        if (response.data.data.length > 0) {
            $.each(response.data.data, function (index, stores) {
                stores_list +=     '<tr>' +
                                            '<td>' +
                                                ...
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + stores.store_name +
                                                '<div class="table-links">' +
                                                    '<a href="master-stores/'+stores.id+'" class="text-primary"><i class="fas fa-eye"></i>View</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="master-stores/'+stores.id+'/edit" class="text-warning"><i class="fas fa-edit"></i>Edit</a>' +
                                                '<div class="bullet"></div>' +
                                                    '<a href="#" id="'+stores.id+'" class="text-danger btn-delete-stores"><i class="fas fa-trash"></i>Trash</a>' +
                                                '</div>' +
                                            '</td>' +
                                            '<td>' + (stores.channels.channel_name ?? '-') + '</td>' +
                                            '<td>' + (stores.warehouses.warehouse_name ?? '-') + '</td>' +
                                            '<td>' + moment(stores.created_at).format('DD MMM YYYY HH:mm') + '</td>' +
                                        '</tr>';
            });

            $('#total_data').html(response.data.total);
            $('#stores-list-field').html(stores_list)
        ...
        $('.btn-delete-stores').on('click', function(e){
            e.preventDefault()
            let store_name = $(this).closest('td').contents().filter(function() { 
                return this.nodeType === 3; // Ensures only text (nodeType 3) is retrieved
            }).text().trim();
            swal({
                title: "Attention",
                text: "Are you sure you want to delete this stores ? \n Stores name : "+ store_name,
                icon: "warning",
                buttons: "Ok",
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status == true){
                                ...
                                setTimeout(function() {
                                    window.location.href = '/master-stores';
                                }, 1000); //
                            }else{
                                iziToast.error({
