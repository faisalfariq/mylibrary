$(document).ready(function(){
    ...
    function validateForm(validationErrors) {
        ...
        Object.keys(validationErrors).forEach(function (fieldName) {
            ...
            if(fieldName == 'channel_id'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += (message.replace('channel id', 'channel') + "<br>");
                });
            }
            if(fieldName == 'warehouse_id'){
                validationErrors[fieldName].forEach( function (message){
                    errorMessage += (message.replace('warehouse id', 'warehouse') + "<br>");
                });
            }
    ...
    $('#form-edit-master-stores').on('submit', function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.status == true){
                    ...
                    setTimeout(function() {
                        window.location.href = '/master-stores/' + store_id;
                    }, 1000); //
                }else{
                    iziToast.error({
